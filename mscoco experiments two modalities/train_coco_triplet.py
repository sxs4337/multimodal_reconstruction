...
def triplet_loss(image_logits, positive_captions_logits, negative_captions_logits, 
                margin = 1.0, scope="triplet_loss"):
    with tf.variable_scope(scope):
        positive_distance = _euc_distance(positive_captions_logits, image_logits)
        negative_distance = _euc_distance(negative_captions_logits, image_logits)
...
def main(cmd_opt):
	...
	image_placeholder = tf.placeholder(dtype=tf.float32, shape=[cmd_opt.batchSize, cmd_opt.imageSize, \
											cmd_opt.imageSize, 3],name="image_placeholder")
	positive_caption_placeholder = tf.placeholder(dtype=tf.float32, shape=[cmd_opt.batchSize, 900])
	negative_caption_placeholder = tf.placeholder(dtype=tf.float32, shape=[cmd_opt.batchSize, 900])
	...
	caption_split = tf.split(reverse_captions_t, num_or_size_splits=20,axis=1)
	...
	# Sent2vec embedding
	pos_gru_embedding_1 = tf.contrib.layers.fully_connected(inputs=positive_caption_placeholder,
													num_outputs=cmd_opt.embeddingSize,
													activation_fn=tf.nn.sigmoid,
													weights_initializer=tf.orthogonal_initializer(),
													biases_initializer=tf.zeros_initializer,
													scope="pos_gru_embedding_1")
													
	pos_gru_embedding_2 = tf.contrib.layers.fully_connected(inputs=pos_gru_embedding_1,
													num_outputs=cmd_opt.embeddingSize,
													activation_fn=tf.nn.sigmoid,
													weights_initializer=tf.orthogonal_initializer(),
													biases_initializer=tf.zeros_initializer,
													scope="pos_gru_embedding_2")
													
	neg_gru_embedding_1 = tf.contrib.layers.fully_connected(inputs=negative_caption_placeholder,
													num_outputs=cmd_opt.embeddingSize,
													activation_fn=tf.nn.sigmoid,
													weights_initializer=tf.orthogonal_initializer(),
													biases_initializer=tf.zeros_initializer,
													scope="neg_gru_embedding_1")
													
	neg_gru_embedding_2 = tf.contrib.layers.fully_connected(inputs=neg_gru_embedding_1,
													num_outputs=cmd_opt.embeddingSize,
													activation_fn=tf.nn.sigmoid,
													weights_initializer=tf.orthogonal_initializer(),
													biases_initializer=tf.zeros_initializer,
													scope="neg_gru_embedding_2")
	...
	# pdb.set_trace()
	# Add Triplet loss
	triplet_loss_t = triplet_loss(image_embedding, pos_gru_embedding_2, neg_gru_embedding_2, cmd_opt.margin)
	...
	loss = cmd_opt.tripletScale * triplet_loss_t

	tf.summary.scalar('Total Loss',loss)
	tf.summary.scalar('Triplet Loss', triplet_loss_t)
	...
	with supervisor.managed_session(config=config_proto) as sess:     
		...
		for iter in range(global_step, 50000):
			...
			image_batch, caption_vecs = sess.run([image_batch_t, gru_fin_encoder])
			negative_batch = np.copy(caption_vecs)
			np.random.shuffle(negative_batch)					   
			_, loss_step, t_loss, summary = sess.run([train_op, loss, 
																triplet_loss_t, summary_op], 
																feed_dict={image_placeholder: image_batch,
																positive_caption_placeholder: caption_vecs, 
																negative_caption_placeholder: negative_batch})
			...
			if (iter+1) % cmd_opt.displayIter == 0:
				...
				print ("Triplet Loss at " , iter+1, ": ", str(t_loss))
...
if __name__ == '__main__':
	...
	parser.add_argument("--tripletScale", default=1.0, type=float, help="Scaling factor for contastive loss")
	parser.add_argument("--margin", default=100, type=int, help="Margin for contrastive loss")
